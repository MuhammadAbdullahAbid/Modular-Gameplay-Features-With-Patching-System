...
#include "MGFImplementation/AbilitySystem/MGFImplementationAbilitySystemComponent.h"
#include "MGFImplementation/AbilitySystem/MGFImplementationAbilityAttributeSet.h"
#include "MGFImplementation/AbilitySystem/AbilityInputBindingComponent.h"

#define LOCTEXT_NAMESPACE "MGFImplementationFeatures"
...
void UGameFeatureAction_AddAbilities::AddAdditionalAssetBundleData(FAssetBundleData& AssetBundleData)
{
	if (UAssetManager::IsValid())
	{
		...
		{
			for (const FMGFImplementationAbilityMapping& Ability : Entry.GrantedAbilities)
			{
			...
			for (const FMGFImplementationAttributesMapping& Attributes : Entry.GrantedAttributes)
			{
...
{
	UE_LOG(LogTemp, Error, TEXT("Added ability"), *Actor->GetPathName());
	if (UAbilitySystemComponent* AbilitySystemComponent = FindOrAddComponentForActor<UAbilitySystemComponent>(Actor, AbilitiesEntry))
	{
		...
		for (const FMGFImplementationAbilityMapping& Ability : AbilitiesEntry.GrantedAbilities)
		{
			if (!Ability.AbilityType.IsNull())
			{
		...
		for (const FMGFImplementationAttributesMapping& Attributes : AbilitiesEntry.GrantedAttributes)
		{
			if (!Attributes.AttributeSetType.IsNull())
			{
				TSubclassOf<UMGFImplementationAbilityAttributeSet> SetType = Attributes.AttributeSetType.LoadSynchronous();
				if (SetType)
				{
